...
- **`articles.raw_content`** and **`articles.formatted_body`**: The formatted body is derived from the raw content. Storing both is denormalized for performance but violates strict normalization.

---

## Migration Notes

The following fixes are planned for the next schema revision:

- [ ] Rename `user_prefernces` → `user_preferences`
- [ ] Rename `articlle_id` → `article_id` in `user_reads`
- [ ] Rename `reigon` → `region` in `summaries`
- [ ] Add `AUTO_INCREMENT` to all single-column primary keys
- [ ] Add explicit `FOREIGN KEY` constraints with appropriate `ON DELETE` / `ON UPDATE` rules
- [ ] Standardize all temporal columns to `TIMESTAMP` or `DATETIME`
- [ ] Evaluate replacing JSON columns (`alert_ids`, `alert_types`, `raw_data`) with junction/relational tables